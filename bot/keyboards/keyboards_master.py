...
from bot.utils.http_appointment import (AvailablAppointment,
                                        get_available_appointment_by_master_id)

from ..utils.http_master import get_master_by_id, get_masters_list
from .collback_factory import (DateCallbackFactory, MastersCallbackFactory,
                               TimeCallbackFactory)
...
async def data_message_for_description_master(master_id):
    ...
    keyboard_date_appointment = await get_keyboard_date_appointment(
        available_appointment_list, master
    )
    return master.description, keyboard_date_appointment


async def get_keyboard_date_appointment(
    available_appointment_list: list[AvailablAppointment], master
            ...
            builder.row(
                types.InlineKeyboardButton(
                    ...
                    callback_data=DateCallbackFactory.from_date_obj(
                        master_id=master.id, date_obj=date_obj
                    ).pack(),
                )
            )
    else:
...
async def get_keyboard_time_appointment(master_id, date) -> types.InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    available_appointment_list = await get_available_appointment_by_master_id(
        master_id, date
    )
