...
from ..utils.model import AvailablAppointment


async def get_available_appointment_all_masters(
    date=None,
) -> list[AvailablAppointment]:
    async with AsyncClient() as client:
        try:
            ...
            return [
                AvailablAppointment(**appointment) for appointment in response.json()
            ]
        except (HTTPError, ValueError, KeyError):
            return None


async def get_available_appointment_by_master_id(
    master_id=None, date=None
) -> list[AvailablAppointment]:
    async with AsyncClient() as client:
        try:
            url = settings.BASE_URL + settings.API_APPOINTMENTS
...
async def get_booking_appointment_by_tg_id(tg_id: int) -> list[AvailablAppointment]:
    async with AsyncClient() as client:
        try:
...
async def book_appointment(tg_id: int, appointment_id: int) -> dict:
    async with AsyncClient() as client:
        try:
            ...
            data = {"appointment_id": appointment_id, "tg_id": int(tg_id)}
...
async def delete_book_appointment(tg_id: int, appointment_id: int) -> dict:
    async with AsyncClient() as client:
        try:
            ...
            data = {"appointment_id": appointment_id, "tg_id": tg_id}
