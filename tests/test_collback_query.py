...
from bot.dispatcher.collback_query import (callbacks_date, callbacks_master,
                                           callbacks_time, delete_edit_book)


@pytest.mark.asyncio
@patch(
    "bot.dispatcher.collback_query.data_message_for_description_master",
    new_callable=AsyncMock,
)
async def test_callbacks_master_masters(mock_data_message_for_description_master):
    mock_data_message_for_description_master.return_value = (
        "Описание мастера",
        "keyboard",
    )
...
@pytest.mark.asyncio
@patch(
    "bot.dispatcher.collback_query.get_keyboard_time_appointment",
    new_callable=AsyncMock,
)
...
async def test_callbacks_master_time_success(
    mock_delete_book_appointment, mock_book_appointment
):
...
@patch("bot.dispatcher.collback_query.book_appointment", new_callable=AsyncMock)
@patch("bot.dispatcher.collback_query.delete_book_appointment", new_callable=AsyncMock)
async def test_callbacks_master_time_with_old_appointment(
    mock_delete_book_appointment, mock_book_appointment
):
    mock_book_appointment.return_value = {}
    mock_delete_book_appointment.return_value = {}
